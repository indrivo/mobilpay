...
use Drupal\commerce_mobilpay\InvoiceItemService;
use Drupal\commerce_mobilpay\ExchangeRateService;

/**
 * Class PaimentInvoiceService.
...
class PaimentInvoiceService {
  ...
  /**
   * Drupal\commerce_mobilpay\InvoiceItemService definition.
   *
   * @var \Drupal\commerce_mobilpay\InvoiceItemService
   */
  protected $commerceMobilpayInvoiceItem;

  /**
   * Drupal\commerce_mobilpay\ExchangeRateService definition.
   *
   * @var \Drupal\commerce_mobilpay\ExchangeRateService
   */
  protected $commerceMobilpayExchangeRate;
  ...
  /**
   * Constructs a new PaimentInvoiceService object.
   */
  public function __construct(
      PaymentAddressService $commerce_mobilpay_payment_address,
      InvoiceItemService $commerce_mobilpay_invoice_item,
      ExchangeRateService $commerce_mobilpay_exchange_rate,
      DOMNode $elem = null) {
    $this->commerceMobilpayPaymentAddress = $commerce_mobilpay_payment_address;
    $this->commerceMobilpayInvoiceItem = $commerce_mobilpay_invoice_item;
    $this->commerceMobilpayExchangeRate = $commerce_mobilpay_exchange_rate;
    if($elem != null) {
	...
	protected function loadFromXml(DOMNode $elem) {
		...
		if($attr == null) {
			throw new  \Exception('PaimentInvoiceService::loadFromXml failed; currency attribute missing', self::ERROR_LOAD_FROM_XML_CURRENCY_ATTR_MISSING); 
		}
		...
		if($elems->length == 1) {
			...
			if($elems->length == 1) {
				$this->billingAddress = $this->commerceMobilpayPaymentAddress($elems->item(0));
			}
			$elems = $addrElem->getElementsByTagName('shipping');
			if($elems->length == 1) {
				$this->shippingAddress = $this->commerceMobilpayPaymentAddress($elems->item(0));
			}
		}
		...
		if($elems->length == 1) {
			...
			if($elems->length > 0) {
				...
				foreach ($elems as $itemElem) {
					try {
						$objItem = $this->commerceMobilpayInvoiceItem($itemElem);
						$this->items[] = $objItem;
		...
		if($elems->length == 1) {
			...
			foreach ($elems as $rateElem) {
				try {
					$objRate = $this->commerceMobilpayExchangeRate($rateElem);
	...
	public function createXmlElement( \DOMDocument $xmlDoc) {
		...
		if(($this->billingAddress instanceof PaymentAddressService) || ($this->shippingAddress instanceof PaymentAddressService)) {
			$xmlAddr = null;
			if($this->billingAddress instanceof PaymentAddressService) {
				try {
			...
			if($this->shippingAddress instanceof PaymentAddressService) {
				try {
		...
		if(is_array($this->exchangeRates) && sizeof($this->exchangeRates) > 0) {
			...
			foreach ($this->exchangeRates as $rate) {
				if(!($rate instanceof ExchangeRateService)) {
					continue;
				}
				try {
	...
	public function setBillingAddress(PaymentAddressService $address) {
		$this->billingAddress = $address;
		
		return $this;
	}
	
	public function setShippingAddress(PaymentAddressService $address) {
	...
	public function addHeadItem(InvoiceItemService $item) {
		array_unshift($this->items, $item);
		
		return $this;
	}
	
	public function addTailItem(InvoiceItemService $item) {
	...
	public function addHeadExchangeRate(ExchangeRateService $rate) {
		array_unshift($this->exchangeRates, $rate);
		
		return $this;
	}
	
	public function addTailExchangeRate(ExchangeRateService $rate) {
