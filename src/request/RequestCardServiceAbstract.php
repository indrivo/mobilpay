...
namespace Drupal\commerce_mobilpay\request;

use Drupal\file\Entity\File;
use Drupal\commerce_mobilpay\RequestCardService;

/**
 * Class RequestCardServiceAbstract
 ...
 */
abstract class RequestCardServiceAbstract {
  ...
  /**
     ...
     * Mandatory for RequestSmsService // Todo: use real class
  ...
  public function __construct () {
    ...
    // Todo: get Identifier from order
  ...
  static public function factory ($data) {
    ...
    if (@$xmlDoc->loadXML($data) === true) {
        ...
        $objPmReq = RequestCardServiceAbstract::_factoryFromXml($xmlDoc);
        $objPmReq->_setRequestInfo(self::VERSION_XML, $data);
    }
    else {
        //try to create payment request from query string
        $objPmReq = RequestCardServiceAbstract::_factoryFromQueryString($data);
  ...
  static public function factoryFromEncrypted ($envKey, $encData, $privateKeyFilePath, $privateKeyPassword = null) {
    $privateKey = null;
    $logger = \Drupal::logger('mobilpay');

    if ($privateKeyPassword == null) {
      $privateKey = openssl_pkey_get_private(file_get_contents($privateKeyFilePath));
    }
    else {
      $privateKey = openssl_pkey_get_private(file_get_contents($privateKeyFilePath), $privateKeyPassword);
    }
    if ($privateKey === false) {
      $logger->notice('Error loading private key: ' . $privateKeyFilePath);
    ...
    if ($result === false) {
      $logger->notice('Failed decrypting data: ' . openssl_error_string());
      throw new  \Exception('Failed decrypting data', self::ERROR_CONFIRM_FAILED_DECRYPT_DATA);
    }

    return RequestCardServiceAbstract::factory($data);
  }

  static protected function _factoryFromXml ( \DOMDocument $xmlDoc) {
      $elems = $xmlDoc->getElementsByTagName('order');
      if ($elems->length != 1) {
        throw new  \Exception('factoryFromXml order element not found', RequestCardServiceAbstract::ERROR_FACTORY_BY_XML_ORDER_ELEM_NOT_FOUND);
      }
      $orderElem = $elems->item(0);

      $attr = $orderElem->attributes->getNamedItem('type');
      if ($attr == null || strlen($attr->nodeValue) == 0) {
        throw new  \Exception('factoryFromXml invalid payment request type=' . $attr->nodeValue, RequestCardServiceAbstract::ERROR_FACTORY_BY_XML_ORDER_TYPE_ATTR_NOT_FOUND);
      }
      switch ($attr->nodeValue) {
        case RequestCardServiceAbstract::PAYMENT_TYPE_CARD:
          $objPmReq = \Drupal::service('commerce_mobilpay.request_card');
          break;
        case RequestCardServiceAbstract::PAYMENT_TYPE_SMS:
          $objPmReq = \Drupal::service('commerce_mobilpay.request_sms');
          break;
          // Comment these case, because mobilPay doesn't have classes, and we don't have this cases.
          // case RequestCardServiceAbstract::PAYMENT_TYPE_ADMIN:
          //   $objPmReq = new Mobilpay_Payment_Request_Admin();
          //   break;
          // case RequestCardServiceAbstract::PAYMENT_TYPE_INFO:
          //   $objPmReq = new Mobilpay_Payment_Request_Info();
          //   break;
          // case RequestCardServiceAbstract::PAYMENT_TYPE_CASH:
          //   $objPmReq = new Mobilpay_Payment_Request_Cash();
          //   break;
          // case RequestCardServiceAbstract::PAYMENT_TYPE_TRANSFER:
          //   $objPmReq = new Mobilpay_Payment_Request_Transfer();
          //   break;
          // case RequestCardServiceAbstract::PAYMENT_TYPE_ING_HOME_PAY:
          //   $objPmReq = new Mobilpay_Payment_Request_IngHomePay();
          //   break;
        default:
          throw new  \Exception('factoryFromXml invalid payment request type=' . $attr->nodeValue, RequestCardServiceAbstract::ERROR_FACTORY_BY_XML_INVALID_TYPE);
          break;
      }
  ...
  protected function _parseFromXml (\DOMElement $elem) {
		$xmlAttr = $elem->attributes->getNamedItem('id');
		if ($xmlAttr == null || strlen((string) $xmlAttr->nodeValue) == 0) {
			// Todo: use real class
			throw new  \Exception('RequestSmsService::_parseFromXml failed: empty order id', self::ERROR_LOAD_FROM_XML_ORDER_ID_ATTR_MISSING);
		}
		...
		if ($elems->length != 1) {
			// Todo: use real class
			throw new  \Exception('RequestSmsService::loadFromXml failed: signature is missing', self::ERROR_LOAD_FROM_XML_SIGNATURE_ELEM_MISSING);
		}
		...
		if ($elems->length == 1) {
			// Todo: use real class
			$this->objPmNotify = \Drupal::service('commerce_mobilpay.request_notify');
    ...
    $pub_key = openssl_pkey_get_public(file_get_contents($x509FilePath));
    $publicKey = $pub_key; // = openssl_pkey_get_details($pub_key);
