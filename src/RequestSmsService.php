...
class RequestSmsService {
  ...
  /**
   * Constructs a new RequestSmsService object.
   */
  public function __construct() {
	...
	protected function _loadFromXml(\DOMElement $elem) {
		...
		if($elems->length != 1) {
			throw new  \Exception('RequestSmsService::loadFromXml failed: service is missing', self::ERROR_LOAD_FROM_XML_SERVICE_ELEM_MISSING);
		}
    ...
    if(!isset($reqParams['signature'])) {
      throw new  \Exception('RequestSmsService::loadFromQueryString failed: signature is missing', self::ERROR_LOAD_FROM_XML_SIGNATURE_ELEM_MISSING);
    }
    $this->signature = $reqParams['signature'];
    if(!isset($reqParams['service'])) {
      throw new  \Exception('RequestSmsService::loadFromQueryString failed: service is missing', self::ERROR_LOAD_FROM_XML_SERVICE_ELEM_MISSING);
    }
    $this->service = $reqParams['service'];
    if(!isset($reqParams['tran_id'])) {
      throw new  \Exception('RequestSmsService::loadFromQueryString failed: empty order id', self::ERROR_LOAD_FROM_XML_ORDER_ID_ATTR_MISSING);
    }
