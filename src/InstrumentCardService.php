...
class InstrumentCardService {
  ...
  /**
   * Constructs a new InstrumentCardService object.
   */
  ...
  protected function loadFromXml(DOMNode $elem) {
    ...
    if ($elems->length != 1) {
      throw new  \Exception('InstrumentCardService::loadFromXml failed: invalid card number', self::ERROR_LOAD_FROM_XML_CARD_ELEM_MISSING);
    }
    ...
    if ($elems->length != 1) {
      throw new  \Exception('InstrumentCardService::loadFromXml failed: invalid exp year', self::ERROR_LOAD_FROM_XML_EXP_YEAR_ELEM_MISSING);
    }
    $xmlElem = $elems->item(0);
    ...
    if ($elems->length != 1) {
      throw new  \Exception('InstrumentCardService::loadFromXml failed: invalid exp month', self::ERROR_LOAD_FROM_XML_EXP_MONTH_ELEM_MISSING);
    }
    $xmlElem = $elems->item(0);
    ...
    if ($elems->length != 1) {
      if (substr($this->number, 1) != 6) {
        throw new  \Exception('InstrumentCardService::loadFromXml failed: invalid cvv2', self::ERROR_LOAD_FROM_XML_CVV2_ELEM_MISSING);
      }
      else {
        $this->cvv2 = '';
      }
